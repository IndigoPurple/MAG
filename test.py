...
parser.add_argument('--ld_num_steps', type=int, default=30) # default=30
...
for data in input_iter(input_dir):
    ...
    with torch.no_grad():
        ...
        for _ in range(args.niters):
            pcl_next, traj = patch_based_denoise(
                model=model,
                pcl_noisy=pcl_next,
    ...
    for i in range(len(traj)):
        traj_denoised = traj[i].cpu()
        traj_denoised = traj_denoised * data['scale'] + data['center']
        save_path = os.path.join(output_dir, 'pcl', 'traj' + data['name'] + str(i) +'.xyz')
        np.savetxt(save_path, traj_denoised.numpy(), fmt='%.8f')
# Evaluate
evaluator = Evaluator(
